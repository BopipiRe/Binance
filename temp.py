...
async def scan_symbol(session, symbol, results, interval):
    ...
    try:
        ...
        if kline and (kline['price_change'] >= 5 and kline['close'] > kline['high_band']) or (kline['price_change'] <= -5 and kline['close'] < kline['lower_band']):
...
async def reset_symbols_have_res():
    ...
    while True:
        ...
        global symbols_list
        async with aiohttp.ClientSession() as session:
            await get_all_futures_symbols(session)
