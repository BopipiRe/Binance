...
async def push_wechat(title, pairs):
    token = "e91fc3d7210641908abc048ccaf6852a"  # 从PushPlus官网获取
...
async def scan_symbol_binance(session, symbol, results):
    ...
    try:
        ...
        if kline and (abs(kline['price_change']) >= 10):
...
async def get_closed_kline_bybit(session, symbol):
    ...
    params = {
        "category": "linear",
        "symbol": symbol,
        "interval": 15,  # Bybit的间隔格式为"15"（15分钟）
        "limit": 2
    }
    async with session.get(url, params=params) as response:
        data = await response.json()
        ...
        prev_kline = klines[1]
        latest_close_time = pd.to_datetime(latest_kline[0], unit="ms") + timedelta(minutes=15)
        current_time = datetime.utcnow()
        ...
        return {
            "symbol": symbol,
            "open_time": pd.to_datetime(res_kline[0], unit="ms"),
            "close_time": pd.to_datetime(res_kline[0], unit="ms") + timedelta(minutes=15),
            "close": close_price,
...
def get_all_futures_symbols_gateio():
    ...
    response = requests.get(url)
    data = response.json()
    return [s["contract"] for s in data if int(s["volume_24h_quote"]) > 500000]  # 大于500000的合约
...
async def get_closed_kline_gateio(session, symbol):
    ...
    params = {
        "contract": symbol,
        "interval": "15m",  # 支持15m、1h等，需转为字符串格式
        "limit": 2
    }
    ...
    async with session.get(url, params=params) as response:
        ...
        prev_kline = data[1]
        latest_close_time = pd.to_datetime(latest_kline["t"], unit="s") + timedelta(minutes=15)
        current_time = datetime.utcnow()
        ...
        return {
            ...
            "close_time": pd.to_datetime(res_kline["t"], unit="s") + timedelta(minutes=15),
            "close": close_price,
            "price_change": price_change,
        }
...
async def scan_high_change_contracts_gateio():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for symbol in symbols_list_gateio:
            ...
            tasks.append(task)
            await asyncio.sleep(0.05)  # 添加请求间隔
...
async def coordinated_scan():
    ...
    while True:
        now = datetime.now()
        next_scan = now.replace(second=0, microsecond=0) + timedelta(minutes=(15 - now.minute % 15))
        ...
        # 扫描完成后立即重置集合
        symbols_have_res.clear()
        print(f"已重置去重集合 | 下次扫描时间: {next_scan + timedelta(minutes=15)}")
...
async def periodic_scan_binance():
    ...
    try:
        ...
        if high_change_klines:
            pairs = [f"{kline['symbol']}: {kline['price_change']:.2f}%" for kline in high_change_klines]
...
if __name__ == "__main__":
    ...
    # print(f"gateio合约数量: {len(symbols_list_gateio)}")
    # # 启动主循环
    # asyncio.run(coordinated_scan())
    asyncio.run(push_wechat("测试", "测试内容"))